...
static bool feed_bytes(MspCodec *codec, const uint8_t *bytes, size_t length,
                       MspFrame *frame)
{
    bool emitted = false;
    for (size_t i = 0; i < length; ++i) {
        emitted = msp_codec_feed(codec, bytes[i], frame) || emitted;
    }
    return emitted;
}
...
void test_msp_codec(void)
{
    ...
    for (size_t i = 0; i < sizeof response - 1u; ++i) {
        assert(!msp_codec_feed(&codec, response[i], &frame));
    }
